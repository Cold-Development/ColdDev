 ...
 * <p>
 * To make executable methods in the command, annotate them with {@link ColdExecutable}. The first parameter will be
 ...
 */
public abstract class BaseColdCommand implements ColdCommand {
    ...
    public BaseColdCommand(ColdPlugin coldPlugin) {
    ...
    public void invoke(CommandContext context) {
        ...
        if (!method.isPresent()) {
            ...
            this.coldPlugin.getLogger().warning("No matching @ColdExecutable method found for command " + this.getCommandInfo().name() + ". Expected arguments (or similar matching optional arguments): [" + arguments + "]");
            return;
        }
    ...
    private boolean verifyMethods(List<Method> methods) {
        if (methods.isEmpty()) {
            this.coldPlugin.getLogger().warning("No @ColdExecutable methods found for command " + this.getCommandInfo().name());
            return false;
        }
        ...
        for (Method method : methods) {
            ...
            if (parameters.length == 0 || parameters[0] != CommandContext.class) {
                this.coldPlugin.getLogger().warning("Invalid @ColdExecutable method for command " + this.getCommandInfo().name() + ". Must have a first parameter of type CommandContext.");
                return false;
            }
        }
    ...
    private static class MethodScore implements Comparable<MethodScore> {
        ...
        @Override
        public int compareTo(BaseColdCommand.MethodScore o) {
